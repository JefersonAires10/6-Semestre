...
def client_request(port, operation):
    try:
...
def run_load_test(port, operation):
    num_clients = 100
    threads = []
    ...
    for _ in range(num_clients):
        thread = Thread(target=client_request, args=(port, operation))
        thread.start()
...
if __name__ == "__main__":
    operation = "+;10;11"  # Ajuste a operação para usar operadores corretos
    print("Testando servidor singlethread na porta 12001")
    run_load_test(12001, operation)
    
    print("Testando servidor multithread na porta 12000")
    run_load_test(12000, operation)